...
async function desaprovarMusica(id) {
    try {
        const url = `/playlist/desaprovarmusica/${id}`;        
...
function mostrarTooltipErro(id) {
    ...
    if (botao) {
        var tooltip = bootstrap.Tooltip.getInstance(botao);
        tooltip._config.title = "Ocorreu um problema ao tentar desaprovar a música.";
        tooltip.update();
        tooltip.show();
        setTimeout(() => {
            tooltip.hide();
...
function recarregarPagina() {
    ...
    //if (trs.length === 0) {
    window.location.href = '/playlist/listagem';
    //}
}
